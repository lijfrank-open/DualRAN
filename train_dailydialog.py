...
from dataloader import DailyDialogueRobertaDataset
from loss import Focal_Loss, MaskedNLLLoss
from model import MyDialogue
...
def get_DailyDialogue_loaders(path='', batch_size=32, classify='emotion', num_workers=0, pin_memory=False):

    trainset = DailyDialogueRobertaDataset(path=path, split='train', classify=classify)
    validset = DailyDialogueRobertaDataset(path=path, split='valid', classify=classify)
    testset = DailyDialogueRobertaDataset(path=path, split='test', classify=classify)
...
def train_or_eval_model(model, loss_function, dataloader, epoch, optimizer=None, train=False, cuda_flag=False):
    ...
    return avg_loss, avg_accuracy, labels, preds, masks, fscores, vids
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='/home/lijfrank/code/dataset/DAILYDIALOG_features/dailydialog_features_roberta.pkl', help='dataset dir')
    ...
    parser.add_argument('--rnn_layer', type=int, default=5, help='')
    parser.add_argument('--attention_head', type=int, default=4, help='')
    parser.add_argument('--attention_layer', type=int, default=6, help='')
    parser.add_argument('--input_size', type=int, default=1024, help='')
    parser.add_argument('--input_in_size', type=int, default=768, help='')
    ...
    if args.classify == 'emotion':
        if args.class_weight:
            ...
            loss_weights = torch.FloatTensor([0.125122598, 0.830965537, 0.009924354, 0.011167325, 0.001689665, 0.017702638, 0.003427883])
    ...
    train_loader, valid_loader, test_loader = get_DailyDialogue_loaders(path=args.data_path,
                                                                        batch_size=batch_size,
                                                                        classify=args.classify,
                                                                        num_workers=0)

    valid_losses, valid_fscores, test_fscores = [], [], []
    ...
    for e in range(n_epochs):
        ...
        if (e+1)%10 == 0:
            ...
            x4 = 'test_best_acc: {}, [test_best_micro w/o Neutral, test_best_macro]: {}'.format(best_acc, best_fscore)
            print(x4)
            lf.write(x4 + '\n')
    ...
    valid_best_micro = np.max(valid_fscores[0])
    valid_best_macro = valid_fscores[1][np.argmax(valid_fscores[0])]

    valid_best_micro_ = valid_fscores[0][np.argmax(valid_fscores[1])]
    valid_best_macro_ = np.max(valid_fscores[1])

    test_best_micro = np.max(test_fscores[0])
    test_best_macro = test_fscores[1][np.argmax(test_fscores[0])]

    test_best_micro_ = test_fscores[0][np.argmax(test_fscores[1])]
    test_best_macro_ = np.max(test_fscores[1])

    print('valid_best_micro w/o Neutral, valid_best_macro:', valid_best_micro, valid_best_macro)
    print('test_best_micro w/o Neutral, test_best_macro:', test_best_micro, test_best_macro_)
    
    scores = [valid_best_micro, valid_best_macro, test_best_micro, test_best_macro]
